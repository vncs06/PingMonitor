    ...
    private readonly Label _lblSource;
        ...
        private async Task DoPing()
        {
            ...
            try
            {
                ...
                {
                    // Use native ICMP to specify source without spawning external process
                    await Task.Yield();
                    var r = IcmpNative.PingIPv4(host, selected.IPAddress, 2000);
                    if (r.success)
                    {
                        UpdateStatus($"Online", Color.ForestGreen);
                        _lblLatency.Text = $"Latência: {r.rttMs} ms";
                    }
                    else
                    {
        ...
        // Removed external ping.exe invocation to reduce AV heuristics
